...
parseDynFlagsFromHsFileHead dflags inputFn = do
  let dflags0 = dflags `gopt_unset` Opt_Haddock `gopt_unset` Opt_KeepRawTokenStream
...
runParserMayPreprocessFromFile p dflags fn = do
  ...
  ebuf <- if LangExt.Cpp `xopt` dflagsWithOptions then liftIO $
    Temp.withTempDirectory (sTmpDir $ settings dflagsWithOptions)
                           "GHC-Compiler-Notes-runParserMayPreprocessFromFile" \tdir -> do
      let outFn = tdir </> takeFileName fn
      r <- (doCpp dflagsWithOptions True fn outFn *> pure Nothing) `catch` \(e :: GhcException) ->
        pure $ Just e
      case r of
        Just e  -> pure $ Left e
        Nothing -> do
          -- avoid lazy I/O
          s <- readFile outFn
          length s `seq` (pure $ Right $ stringToStringBuffer s) else do
...
doCpp dflags raw input_fn output_fn = do
  ...
  let target_defs = [ "-D" ++ HOST_OS ++ "_BUILD_OS"
  ...
  let sse_defs = ["-D__SSE__" | isSseEnabled dflags] ++ ["-D__SSE2__" | isSse2Enabled dflags]
        ++ ["-D__SSE4_2__" | isSse4_2Enabled dflags]

  let avx_defs = ["-D__AVX__" | isAvxEnabled dflags] ++ ["-D__AVX2__" | isAvx2Enabled dflags]
  ...
  mb_macro_include <- if not (null pkgs) && gopt Opt_VersionMacros dflags then do
    ...
    -- comment 8.
    return [SysTools.FileOption "-include" macro_stub] else return []

  cpp_prog (map SysTools.Option verbFlags ++ map SysTools.Option include_paths
            ++ map SysTools.Option hsSourceCppOpts ++ map SysTools.Option target_defs
            ++ map SysTools.Option backend_defs ++ map SysTools.Option th_defs
            ++ map SysTools.Option hscpp_opts ++ map SysTools.Option sse_defs
            ++ map SysTools.Option avx_defs ++ mb_macro_include
            ++ [ SysTools.Option "-x"
...
getOptionsFromTokenStream dflags = parseToksWithFullArg
  where
    ...
    parseToks open close
      | IToptions_prag str <- getToken open, ITclose_prag <- getToken close = case toArgs str of
        Left err   -> panic ("getOptions'.parseToks: " ++ err)
        Right args -> (map (L (getLoc open)) args ++)
          <$> parseToksWithFullArg
    parseToks open close
      | ITinclude_prag str <- getToken open, ITclose_prag
        <- getToken close = (map (L (getLoc open)) ["-#include", removeSpaces str] ++)
        <$> parseToksWithFullArg
    parseToks open close
      | ITdocOptions str <- getToken open, ITclose_prag
        <- getToken close = (map (L (getLoc open)) ["-haddock-opts", removeSpaces str] ++)
        <$> parseToksWithFullArg
    parseToks open rarg1
      | ITlanguage_prag <- getToken open = parseLanguage rarg1
    parseToks comment rarg1 -- Skip over comments
      | isComment (getToken comment) = parseToksWith1Arg rarg1
    parseToks _ _ = pure []

    parseLanguage (L loc1 (ITconid fs)) = (checkExtension dflags (L loc1 fs) :)
      <$> do
        pToken >>= \case
...
checkExtension dflags (L l ext)
  ...
  = let ext' = unpackFS ext in if ext' `elem` supportedLanguagesAndExtensions
    then L l ("-X" ++ ext') else unsupportedExtnError dflags l ext'

unsupportedExtnError :: DynFlags -> SrcSpan -> String -> a
unsupportedExtnError dflags loc unsup = throw $ mkSrcErr $ unitBag $ mkPlainErrMsg dflags loc $
  text "Unsupported extension: " <> text unsup $$if null suggestions then Outputable.empty
...
generatePackageVersionMacros pkgs =
  concat
...
generateMacros prefix name version =
  ...
  where
    ...
    (major1, (major2, (minor, _))) = let vs = map show $ versionBranch version
