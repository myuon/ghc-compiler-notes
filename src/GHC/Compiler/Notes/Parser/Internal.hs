...
initialCollectedNotesCtx =
  CollectedNotesCtx { cnCtxCollectedNotes = mempty, cnCtxCommentIndent = -1 }

addNoteByCollectingInState :: Monad m => Located Note -> StateT CollectedNotesCtx m ()
addNoteByCollectingInState n =
  modify' \ctx -> ctx { cnCtxCollectedNotes = addNoteByCollecting n $ cnCtxCollectedNotes ctx }

type CollectNotesStateParser o = StateT CollectedNotesCtx (ConduitT (Located Token) o Parser)
...
stripIndentedLineComment :: Monad m
                         => Located String
                         -> StateT CollectedNotesCtx m (Located String)
stripIndentedLineComment (L p s) = case s of
  ...
  where
    ...
    goEnd n ns  = do
      modify' \ctx -> ctx { cnCtxCommentIndent = n }
      pure $ L p ns

endLineCommentContent :: Monad m => StateT CollectedNotesCtx m ()
endLineCommentContent = modify' \ctx -> ctx { cnCtxCommentIndent = -1 }
...
noteTitleMatcher = "Note [" *> Regex.few Regex.anySym
  <* "]"
  <* spacesMatcher

noteTitleSectionLineMatcher :: Regex.RE Char String
noteTitleSectionLineMatcher = lineMatcher
  <* spacesMatcher
  where
...
parseBlockComment mctx s
  | isPragmaBlockComment $ unLoc s = case mctx of
    ...
    Just ctx -> do
      ...
      cont Nothing
  | otherwise = runConduit $ sourceBlockCommentLines s .| sinkBlockCommentContents cont mctx
  where
...
sinkBlockCommentContents cont = goStart
  where
    ...
    sectionLineMatcher = some (Regex.sym '*')
      <* spacesMatcher
...
sinkParsingNoteComment ctx = lift await >>= \case
  ...
  where
    ...
    removeNamedTag' sl el (c:s) = let sl' = advanceSrcLocFromChar sl c in case c of
      '\n' -> L (mkSrcSpan sl' el) s
      _    -> removeNamedTag' sl' el s

completeParsingNote :: ParsingCtx -> CollectNotesStateParser o ()
completeParsingNote (L p buf) = addNoteByCollectingInState $ L p $
  Note { noteId = noteIdFromBuf, noteContent = noteContentFromBuf }
  where
    noteIdFromBuf      = let firstLine = Text.takeWhile (/= '\n') buf
      in case Regex.match noteTitleMatcher $ Text.unpack firstLine of
        Nothing    -> error "unreachable"
        Just title -> NoteId $ Text.pack title

    noteContentFromBuf =
      let skipTwoLine c = do
            i <- get
